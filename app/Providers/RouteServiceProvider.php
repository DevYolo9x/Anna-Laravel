...
use App\Http\Controllers\project\frontend\CategoryController as CategoryControllerProject;
use App\Http\Controllers\project\frontend\ProjectController;
            ...
            if (!empty($segments)) {
                ...
                if (!empty($checkURL)) {
                    if (in_array($first, config('app.alt_langs'))) {
                        ...
                        if ($checkURL->module == 'category_projects') {
                            Route::get('/' . $first . '/{slug}', [CategoryControllerProject::class, 'index'])->middleware(['web', 'locale'])->where(['slug' => '.+'])->name('routerURL');
                        }
                        if ($checkURL->module == 'projects') {
                            Route::get('/' . $first . '/{slug}', [ProjectController::class, 'index'])->middleware(['web', 'locale'])->where(['slug' => '.+'])->name('routerURL');
                        }
                    } else {
                        if ($checkURL->module == 'category_products') {
                        ...
                        if ($checkURL->module == 'category_projects') {
                            Route::get('/{slug}', [CategoryControllerProject::class, 'index'])->middleware(['web', 'locale'])->where(['slug' => '.+'])->name('routerURL');
                        }
                        if ($checkURL->module == 'projects') {
                            Route::get('/{slug}', [ProjectController::class, 'index'])->middleware(['web', 'locale'])->where(['slug' => '.+'])->name('routerURL');
                        }
                    }
                } else {
                    if (in_array($first, config('app.alt_langs'))) {
