...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $type = $request->type;
        $product = Product::select('id', 'title', 'slug', 'price','code', 'price_sale', 'price_contact', 'unit', 'inventory', 'inventoryPolicy', 'inventoryQuantity', 'catalogue_id', 'image', 'ships')
            ->with('product_stocks')
            ->find($id);
        if (!$product) {
            ...
            if ($request->rowid and $quantity) {
                ...
                if (!empty($cart[$request->rowid]["options"])) {
                    $getVersionProduct = ProductVersion::select('id', '_stock_status', '_stock', '_outstock_status')
                        ...
                        ->where('id_version',  $cart[$request->rowid]["options"]['id_version'])
                        ->with('product_stocks')
                        ->first();
                    if (!$getVersionProduct) {
                        $alert['error'] = trans('index.ProductVersionDoesNotExist');
                    ...
                    $product = Product::select('id', 'inventory', 'inventoryPolicy', 'inventoryQuantity')->with('product_stocks')->find($cart[$request->rowid]['id']);
                    //check stock
                    $checkCartVariable = checkCart($product, $quantity);
                    if (!empty($checkCartVariable['status'])) {
                        $alert['error'] = $checkCartVariable['message'];
            ...
            if (isset($cart)) {
                foreach ($cart as $key => $value) {
                    if (!empty($value['options'])) {
                        $getVersionproduct = \App\Models\ProductVersion::select('id', '_stock_status as inventory',  '_outstock_status as inventoryPolicy')
                            ->with('product_stocks')
                            ->where('product_id', $value["id"])
                            ->where('id_version',  $value["options"]['id_version'])->first();
                        $stock = $getVersionproduct->product_stocks->sum('value');
                        //quản lý kho hàng - không cho đặt hàng khi hết hàng - số lượng tồn kho = 0
                        if ($getVersionproduct['inventory'] == 1 && $getVersionproduct['inventoryPolicy']  == 0 && $stock == 0) {
                        ...
                        if ($getVersionproduct['inventory'] == 1 && $getVersionproduct['inventoryPolicy']  == 0) {
                            if ($value['quantity'] > $stock) {
                                array_push($arrStock, trans('index.productMaxVersion', ['title' => $value['title'], 'options' => $value['options'], 'stock' => $stock]));
                            }
                        }
                    } else {
                        $product = \App\Models\Product::select('id', 'inventory', 'inventoryPolicy')->with('product_stocks')->find($value["id"]);
                        $stock = $product->product_stocks->sum('value');
                        //quản lý kho hàng - không cho phép đặt hàng - số lượng tồn kho = 0
                        if ($product['inventory'] == 1 && $product['inventoryPolicy']  == 0 && $stock == 0) {
                            array_push($arrStock, trans('index.productMaxOutStock', ['title' => $value['title']]));
                        }
                        //check số lượng so với tồn kho
                        if ($product['inventory'] == 1 && $product['inventoryPolicy']  == 0) {
                            if ($value['quantity'] > $stock) {
                                array_push($arrStock, trans('index.productMax', ['title' => $value['title'], 'stock' => $stock]));
                            }
                        }
                    }
                }
