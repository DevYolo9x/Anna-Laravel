...
class CategoryController extends Controller
{
    ...
    public function index(Request $request, $slug = "", $id = 0)
    {
        ...
        $data =  Product::join('catalogues_relationships', 'products.id', '=', 'catalogues_relationships.moduleid')
            ...
            ->select('category_products.title as titleC', 'category_products.slug as slugC', 'products.id', 'products.title', 'products.slug', 'products.image', 'products.price', 'products.price_sale', 'products.code', 'products.price_contact', 'products.image_json');
        if (!empty($detail->id)) {
        ...
        $module = 'category';
        $view = 'product.frontend.category.index';
        $data_child = [];
        if( $detail['rgt'] - $detail['lft'] > 1 ){
            $view = 'product.frontend.category.haschild';
            $data_child = CategoryProduct::where(['publish' => 0, 'alanguage' => config('app.locale'), 'parentid' => $detail->id])->orderBy('order', 'ASC')->orderBy('id', 'desc')->get();
            if( !empty($data_child) && count($data_child) > 0 ){
                foreach ( $data_child as $key => $item ){
                    $data_child[$key]['data'] = \App\Models\Product::join('catalogues_relationships', 'products.id', '=', 'catalogues_relationships.moduleid')
                        ->where('catalogues_relationships.module', '=', 'products')
                        ->with('fields')
                        ->where('catalogues_relationships.catalogueid', '=', $item->id)->limit(8)->orderBy('order', 'asc')
                        ->orderBy('id', 'desc')->get();
                }
            }
        }

        return view($view, compact('fcSystem', 'module', 'detail', 'seo', 'data', 'breadcrumb', 'data_child', 'attributes', 'brandFilter'));
    }
    //Page: Sản phẩm
    ...
    {
        $data =  Product::select('category_products.title as titleC', 'category_products.slug as slugC', 'products.id', 'products.code', 'products.title', 'products.slug', 'products.price', 'products.price_sale', 'products.price_contact', 'products.image')
            ->where(['products.alanguage' => config('app.locale'), 'products.publish' => 0]);
        ...
        foreach ($data as $k => $item) {
            $html .= '<div class="w-1/2 md:w-1/3 px-[10px]">' . htmlItemProduct($item, 'item item-product mb-[15px] md:mb-[30px]') . '</div>';
        }
        $paginate .= $data->links();
