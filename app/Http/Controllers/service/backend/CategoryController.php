...
class CategoryController extends Controller
{
    ...
    {
        $module = $this->table;
        $data =  CategoryService::where('alanguage', config('app.locale'))->with('user')->with('listService')->orderBy('lft', 'ASC');
        ...
        $data =  $data->select('id', 'title', 'slug', 'userid_created', 'created_at', 'publish', 'order', 'ishome', 'isservice', 'highlight', 'isaside', 'isfooter', 'lft', 'rgt', 'level');
        ...
        return view('service.backend.category.index', compact('data', 'module', 'configIs'));
    }
    public function create()
    {
        ...
        return view('service.backend.category.create', compact('module', 'htmlCatalogue', 'field'));
    }
    public function store(Request $request)
    {
        ...
        if (!empty($request->file('image'))) {
            $image_url = uploadImageNone($request->file('image'), 'danh-muc-du-an');
        } else {
            $image_url = $request->image_old;
        }
        if (!empty($request->file('banner'))) {
            $banner_url = uploadImageNone($request->file('banner'), 'danh-muc-du-an/banner');
        } else {
        ...
        return redirect()->route('category_services.index')->with('success', "Thêm mới danh mục thành công");
    }
    public function edit($id)
    {
        $module = $this->table;
        $detail  = CategoryService::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('category_services.index')->with('error', "Danh mục không tồn tại");
        }
        $htmlCatalogue = $this->nestedsetbie->dropdown([], config('app.locale'));
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('service.backend.category.edit', compact('detail', 'htmlCatalogue', 'module', 'field'));
    }
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('category_services.index')->with('success', "Cập nhập danh mục thành công");
    }
    public function submit($request = [], $action = '', $id = 0, $arrayImg = [])
    {
        ...
        if ($action == 'create') {
            $id = CategoryService::insertGetId($_data);
        } else {
            CategoryService::find($id)->update($_data);
        }
        if (!empty($id)) {
            //xóa khi cập nhập
            if ($action == 'update') {
