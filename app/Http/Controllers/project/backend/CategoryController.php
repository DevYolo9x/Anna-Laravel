...
class CategoryController extends Controller
{
    ...
    {
        $module = $this->table;
        $data =  CategoryProject::where('alanguage', config('app.locale'))->with('user')->with('listProject')->orderBy('lft', 'ASC');
        ...
        return view('project.backend.category.index', compact('data', 'module', 'configIs'));
    }
    public function create()
    {
        ...
        return view('project.backend.category.create', compact('module', 'htmlCatalogue', 'field'));
    }
    public function store(Request $request)
    {
        ...
        return redirect()->route('category_projects.index')->with('success', "Thêm mới danh mục thành công");
    }
    public function edit($id)
    {
        $module = $this->table;
        $detail  = CategoryProject::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('category_projects.index')->with('error', "Danh mục không tồn tại");
        }
        $htmlCatalogue = $this->nestedsetbie->dropdown([], config('app.locale'));
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('project.backend.category.edit', compact('detail', 'htmlCatalogue', 'module', 'field'));
    }
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('category_projects.index')->with('success', "Cập nhập danh mục thành công");
    }
    public function submit($request = [], $action = '', $id = 0, $arrayImg = [])
    {
        ...
        if ($action == 'create') {
            $id = CategoryProject::insertGetId($_data);
        } else {
            CategoryProject::find($id)->update($_data);
        }
        if (!empty($id)) {
            //xóa khi cập nhập
            if ($action == 'update') {
